...
class TopicInfoOptionType(Enum):
    LABEL = auto()
    PUBLISHER = auto()
    SUBSCRIBER = auto()
...
class TopicInfo(ListBox):
    ...
    """
    def __init__(self, height: int, model: TopicInfoModel=None):
    ...
    def _model_to_options(self, model: TopicInfoModel) -> List[Tuple[str, TopicInfoOption]]:
        ...
        self._append_topics_to_options(model.subscribers, TopicInfoOptionType.SUBSCRIBER, options)
        self._append_label_option("Publishers:", options)
        self._append_topics_to_options(model.publishers, TopicInfoOptionType.PUBLISHER, options)
        return options
